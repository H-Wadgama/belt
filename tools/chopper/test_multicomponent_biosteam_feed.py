...
canonical conversion to component kmol/hr.

State dicts are built via `multicomponent_feed_state.apply_user_update`
(the real measurement-record shape -- one record per fact, see
tools/multicomponent-distillation-dialogue-robustness-plan.md) rather than
hand-rolled flat dict literals, since `apply_user_update` is the only
supported way to construct a valid feed state.
...
def _state(**fields):
    return apply_user_update(empty_feed_state(), fields)
...
def test_weight_percent_composition_plus_molar_total_flow_regression():
    """20/20/60 wt% Water/Methanol/Ethanol at a 100 kmol/hr total flow."""
    state = _state(
